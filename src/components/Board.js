...
export default function Board(props) {
  ...
  const makeMove = () => {
    let bestMove = findBestMove(board, props.player, props.opponent);
    let copyboard = [...board];
    copyboard[bestMove.row][bestMove.col] = props.opponent;
    setBoard(copyboard);
    if (checkWin(props.player) === true) props.setStatus(props.player);
    if (checkWin(props.opponent) === true) props.setStatus(props.opponent);
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        if (board[i][j] === "") return;
      }
